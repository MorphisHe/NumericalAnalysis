...
Formula: 
    ...
    - I = h/2 * sum of (f(an)+f(bn)) for n in [0, N-1]
Parameters:
...
'''
def trap_integration(a, b, N, float_percision=4, print_iter=False):
    h = (b-a) / N
    ...
    for n in range(N):
        ...
        f_an = f(an)
        f_bn = f(bn)
        integral_f += (f_an+f_bn)

        # print intermediate process
        if print_iter:
            print("Num Iteration:", num_iter)
            print("an, bn:", an, bn)
            print("f(an), f(bn):", f_an, f_bn)
            print("Cumulative Integral:", round(h/2 * integral_f, float_percision))
            print("==========================")

        num_iter += 1

    return round(h/2 * integral_f, float_percision), num_iter
...
# get the root
integral, iters = trap_integration(arg_a, arg_b, arg_N, float_percision=arg_percision, print_iter=arg_print)
print("\n===============================================================")
